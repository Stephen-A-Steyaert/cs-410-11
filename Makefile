...
dev: ## Run Flask development server locally
	cd website && PORT=9000 uv run python main.py
...
logs: ## View logs from production containers
	docker compose -f docker-compose.production.ghcr.yml logs -f

logs-web: ## View logs from Flask web container
	docker compose -f docker-compose.production.ghcr.yml logs -f web

logs-traefik: ## View logs from Traefik container
...
shell: ## Open Python shell with Flask app context
	cd website && uv run python

routes: ## Show all Flask routes
	cd website && uv run flask routes

clean-deps: ## Clean and reinstall dependencies
	cd website && rm -rf .venv && uv sync

check-version: ## Check Flask version
	cd website && uv run python -c "import flask; print(flask.__version__)"
